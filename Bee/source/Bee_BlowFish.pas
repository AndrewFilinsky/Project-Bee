...
/// TBlowFish
  ...
  begin
    PInput := @Input;
    Result := ((S[1,PInput^[0]] + S[2,PInput^[1]] {MOD HighestNumber})
      xor S[3,PInput^[2]]) + S[4,PInput^[3]] { MOD HighestNumber};
  ...
  begin
    Result := Count mod 8;
    if Result = 0 then Result := Count else Result := Count + (8 - Result);
